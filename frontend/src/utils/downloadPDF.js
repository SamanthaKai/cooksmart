...
import FAVICON_SVG from "../../public/images/favicon.svg?raw";
...
const TERRACOTTA  = [139, 69, 19];
const HEADER_H    = 30;   // mm — tall enough for logo + two text lines

// Rasterise an SVG string to a PNG data-URL via an off-screen canvas.
function svgToPng(svgString, size = 128) {
...
function checkY(doc, y, needed = 10) {
  if (y + needed > PAGE_H - 14) return addPage(doc);
  return y;
}
...
export async function downloadRecipePDF(recipe, type = "db") {
  const doc = new jsPDF({ unit: "mm", format: "a4" });

  const faviconPng = await svgToPng(FAVICON_SVG, 128);
  ...
  // Favicon — 13×13 mm, vertically centred in the header block
  if (faviconPng) {
    doc.addImage(faviconPng, "PNG", MARGIN, (HEADER_H - 13) / 2, 13, 13);
  }

  const textX = MARGIN + 16;

  // "CookSmart" — large bold white
  doc.setFont("helvetica", "bold");
  doc.setFontSize(20);
  doc.setTextColor(255, 255, 255);
  doc.text("CookSmart", textX, 13);

  // Subtitle — smaller, warm off-white
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  doc.setTextColor(255, 230, 200);
  doc.text("African & Ugandan Recipe Discovery", textX, 21);
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.setFontSize(8);
    doc.setTextColor(255, 255, 255);
    doc.text("Generated by CookSmart | African & Ugandan Cuisine", MARGIN, PAGE_H - 4);
